...
def q_cs(y, T):
    ...
    return torch.clamp(q_val, min=-1e6, max=1300000)  # bornes amples mais raisonnables
...
def q_ci(y, T):  # Convection du lit de braises
    Re = Re_D()
    return torch.clamp((0.911 * s * k_b * Re**0.385 * Pr**(1 / 3)) / D * (T_b - T) * torch.exp(-0.25 * s * y), min=0, max=1300000)
...
class JointModel(nn.Module):
    def __init__(self, lambda_chi=1000.0, R_min=0.0, R_max=0.5):
        super().__init__()
...
# ------------------------------------------------------------------
def train_joint_model(n_epochs=5000, lambda_chi=1000.0, lr=1e-3):
    ...
    for epoch in range(n_epochs):
        ...
        loss_ic = 1_0000 * ((T_ic - T_inf)**2 + (Mw_ic - Mw_inf)**2)
...
# ------------------------------------------------------------------
model = train_joint_model(n_epochs=5000, lambda_chi=20.0)
...
q_rs_vol = q_rs(y).detach().cpu().numpy().flatten() / l_f * 1e-3
q_ri_vol = q_ri(y).detach().cpu().numpy().flatten() / l_f * 1e-3
q_pr_vol = q_pr(y, T).detach().cpu().numpy().flatten() / l_f * 1e-3
q_cs_vol = q_cs(y, T).detach().cpu().numpy().flatten() / l_f * 1e-3
q_ci_vol = q_ci(y, T).detach().cpu().numpy().flatten() / l_f * 1e-3
...
T_values, _ = model(y_values)

print("  y (m) | T(y) (K) |  q_rs  |  q_cs  |  q_ri  |  q_ci  |  q_pr   | Total")
print("------------------------------------------------------------------------")
for y_i, T_i in zip(y_values, T_values):
    q_rs_val = q_rs(y_i).item() / l_f * 1e-3
    q_cs_val = q_cs(y_i, T_i).item() / l_f * 1e-3
    q_ri_val = q_ri(y_i).item() / l_f * 1e-3
    q_ci_val = q_ci(y_i, T_i).item() / l_f * 1e-3
    q_pr_val = q_pr(y_i, T_i).item() / l_f * 1e-3
    total = q_rs_val + q_cs_val + q_ri_val + q_ci_val + q_pr_val
    print(f"{y_i.item():6.1f} | {T_i.item():7.1f} | {q_rs_val:6.1f} | {q_cs_val:6.1f} | {q_ri_val:6.1f} | {q_ci_val:6.1f} | {q_pr_val:7.1f} | {total:6.1f}")
